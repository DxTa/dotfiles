...
beautiful.init(awful.util.getdir("config") .. "/themes/2013/theme.lua")
...
for s = 1, screen.count() do
  ...
  -- Create the wibox
  mywibox[s] = awful.wibox({ position = "top", screen = s, height = beautiful.bar_height })
...
globalkeys = awful.util.table.join(
  ...
  awful.key({ modkey, "Control" }, "q", quit_session_cleanup),
  ...
  awful.key({ modkey, "Shift" }, "e", function ()
    run_or_raise("emacsclient -n -c", { class = "Emacs" })
  end),
  ...
  awful.key({ modkey, "Shift" }, "v", function ()
    run_or_raise("gvim -f", { class = "Gvim" })
  end),
...
awful.rules.rules = {
  ...
  {
    rule = { class = "VirtualBox", name = "*[Running]*" },
    properties = { floating = true },
    callback = function (c)
